...
This allows you to block one incoming attack.
Blocking an attack this way still uses an <<item-use-token, item use token>>.
You can still use <<shield-block, Shield Block>> for yourself during this round.
- [[shield-block]]Shield Block: Negate one attack completely at the cost of 1 <<item-use-token, item use token>>.
Can be used once per round as a <<short-action, short action>>.
...
On success, block all damage they receive from one attack at the cost of 1 <<item-use-token, item use token>>.
...
== Rank II
- Shield Block: You can now negate attacks from *two* enemies simultaneously at the cost of 1 <<item-use-token, item use token>> for both.
...
- Guard: You can protect up to *two* adjacent allies at the cost of 1 <<item-use-token, item use token>> for both.
- Guard: You can protect target from *two* simultaneous attacks at the cost of 1 <<item-use-token, item use token>> for both.
